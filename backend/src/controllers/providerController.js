...
const updateProvider = async (req, res) => {
  ...
  try {
    ...
    if (Object.keys(userFields).length > 0) {
      ...
      if (!updatedUser) {
        return res.status(constant.HTTP_STATUS.NOT_FOUND).json({ message: "User not found" });
      }
    }
    ...
    res.status(constant.HTTP_STATUS.OK).json({ message: "Provider updated successfully" });
  } catch (err) {
...
const updateProviderServices = async (req, res) => {
  ...
  if (!Array.isArray(services_array)) {
    return res.status(constant.HTTP_STATUS.BAD_REQUEST).json({ message: "services_array must be an array" });
  }
  ...
  try {
    ...
    const currentServices = Array.isArray(provider.services_array) ? provider.services_array : [];

    // 2. Find newly added service IDs
    const newServices = services_array.filter(id => !currentServices.includes(id));

    // 3. Update provider's services_array
    await db.update(constant.DB_TABLES.PROVIDERS, { services_array }, 'WHERE user_id = $1', [user_id]);
    ...
    for (const serviceId of newServices) {
      ...
      if (service) {
        const providersArr = Array.isArray(service.providers_array) ? service.providers_array : [];
        if (!providersArr.includes(user_id)) {
          providersArr.push(user_id);
          await db.update(constant.DB_TABLES.SERVICES, { providers_array: providersArr }, 'WHERE service_id = $1', [serviceId]);
        }
      }
    }

    res.status(constant.HTTP_STATUS.OK).json({ message: "Provider services updated successfully" });
  } catch (err) {
    res.status(constant.HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ message: "Internal server error" });
  }
};
...
const softDeleteProvider = async (req, res) => {
  ...
  try {
    ...
    if (updatedUser) {
      res.status(constant.HTTP_STATUS.OK).json({ message: "Provider soft deleted successfully" });
    } else {
      res.status(constant.HTTP_STATUS.NOT_FOUND).json({ message: "User Not Found" });
    }
  } catch (err) {
...
const hardDeleteProvider = async (req, res) => {
  ...
  try {
    ...
    if (deletedProvider && deletedUser) {
      ...
      const services = await db.getAll(constant.DB_TABLES.SERVICES, '', []);
      const servicePromises = services.map(async (service) => {
        if (Array.isArray(service.providers_array) && service.providers_array.includes(provider_id)) {
          const updatedProvidersArray = service.providers_array.filter(id => id !== provider_id);
          await db.update(constant.DB_TABLES.SERVICES, { providers_array: updatedProvidersArray }, 'WHERE service_id = $1', [service.service_id]);
        }
      });
      await Promise.all(servicePromises);

      res.status(constant.HTTP_STATUS.OK).json({ message: "Provider hard deleted successfully" });
    } else {
      res.status(constant.HTTP_STATUS.NOT_FOUND).json({ message: "User Not Found" });
    }
  } catch (err) {
