...
const ChatRoomPopover: React.FC<ChatRoomPopoverProps> = ({ chatId, onBack }) => {
  ...
  return (
    <Box sx={{ width: 320, maxWidth: '100%' }}>
      <Paper elevation={2} sx={{ p: 1, mb: 1, display: "flex", alignItems: "center", gap: 1 }}>
        <IconButton size="small" onClick={onBack}>
          <ArrowBackIcon />
        </IconButton>
        <Avatar src={chat.avatar} alt={chat.name} sx={{ width: 32, height: 32 }} />
        <Typography variant="subtitle1" sx={{ fontWeight: 600, ml: 1 }}>{chat.name}</Typography>
      </Paper>
      <Paper elevation={1} sx={{ maxHeight: 260, overflowY: "auto", mb: 1 }}>
        <List>
          {messages.map((msg, idx) => (
            <ListItem key={idx} sx={{ justifyContent: msg.from === "You" ? "flex-end" : "flex-start" }}>
              ...
              <ListItemText
                ...
                sx={{ textAlign: msg.from === "You" ? "right" : "left", maxWidth: 220 }}
              />
            </ListItem>
          ))}
